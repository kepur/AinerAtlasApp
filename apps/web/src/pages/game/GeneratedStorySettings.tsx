import { useNavigate } from "react-router-dom";
import { ChevronLeft, Play, Edit3, Sparkles, BookOpen, User, Book, Target } from "lucide-react";
import { motion } from "framer-motion";

export default function GeneratedStorySettings() {
  const navigate = useNavigate();

  return (
    <motion.div 
      initial={{ opacity: 0, x: 20 }}
      animate={{ opacity: 1, x: 0 }}
      exit={{ opacity: 0, x: -20 }}
      className="premium w-full min-h-screen bg-[#fdf2f8] flex flex-col font-sans relative text-[#111827] pb-24"
    >
      {/* Header */}
      <header className="sticky top-0 left-0 w-full z-50 px-4 pt-[env(safe-area-inset-top,20px)] h-14 flex items-center justify-between bg-[#fdf2f8]/90 backdrop-blur-md">
        <button onClick={() => navigate(-1)} className="w-8 h-8 flex items-center justify-center text-gray-700 active:scale-95 transition-transform">
          <ChevronLeft size={24} />
        </button>
        <h1 className="font-bold text-[16px] text-[#111827]">故事设定</h1>
        <button className="w-8 h-8 flex items-center justify-center text-pink-500">
          <Play size={20} />
        </button>
      </header>

      <main className="flex-1 px-4 pt-2 flex flex-col gap-4">
        
        {/* Title Card */}
        <div className="bg-white/80 backdrop-blur-md rounded-3xl p-5 shadow-sm border border-white flex flex-col items-center text-center relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-pink-100 rounded-full blur-3xl opacity-50 -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
          
          <div className="w-16 h-16 bg-gradient-to-br from-pink-100 to-rose-100 rounded-2xl flex items-center justify-center mb-3 shadow-inner border border-white">
            <Sparkles size={28} className="text-pink-500" />
          </div>
          <h2 className="text-2xl font-black text-[#111827] mb-2 tracking-wide">青云重生</h2>
          <div className="flex gap-1.5 flex-wrap justify-center">
            <span className="bg-gray-100 text-gray-600 text-[10px] px-2 py-0.5 rounded-full font-medium">仙侠剧情</span>
            <span className="bg-gray-100 text-gray-600 text-[10px] px-2 py-0.5 rounded-full font-medium">逆袭打脸</span>
            <span className="bg-gray-100 text-gray-600 text-[10px] px-2 py-0.5 rounded-full font-medium">师兄妹</span>
          </div>
        </div>

        {/* Story Outline */}
        <div className="bg-white/80 backdrop-blur-md rounded-3xl p-5 shadow-sm border border-white relative">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-6 h-6 rounded-full bg-pink-100 flex items-center justify-center">
              <BookOpen size={12} className="text-pink-500" />
            </div>
            <h3 className="font-bold text-[15px] text-[#111827]">故事主线</h3>
          </div>
          <p className="text-[13px] text-gray-600 leading-relaxed">
            你本是青云宗天资卓绝的大师兄，却遭奸人诬陷勾结魔道，被废去修为逐出师门。五年后，你在一处秘境中获得了上古传承，不仅恢复了修为，还更胜从前。
            <br/><br/>
            如今你隐姓埋名回到青云山下，恰逢宗门大比，你决定在比武大会上揭露当年的真相，拿回属于你的一切...
          </p>
        </div>

        {/* Character Settings */}
        <div className="flex flex-col gap-3">
          <div className="bg-white/80 backdrop-blur-md rounded-2xl p-4 shadow-sm border border-white flex gap-3 items-start">
            <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center shrink-0 border border-indigo-100">
              <User size={14} className="text-indigo-500" />
            </div>
            <div>
              <h4 className="font-bold text-[13px] text-[#111827] mb-0.5">你的设定</h4>
              <p className="text-[11px] text-gray-500 leading-snug">修为尽失的落魄天才，如今带着上古传承归来。性格沉稳内敛，不露锋芒。</p>
            </div>
          </div>

          <div className="bg-white/80 backdrop-blur-md rounded-2xl p-4 shadow-sm border border-white flex gap-3 items-start">
            <div className="w-8 h-8 rounded-full bg-pink-50 flex items-center justify-center shrink-0 border border-pink-100">
              <span className="text-[14px]">🌸</span>
            </div>
            <div>
              <h4 className="font-bold text-[13px] text-[#111827] mb-0.5">对方设定</h4>
              <p className="text-[11px] text-gray-500 leading-snug">小师妹苏灵儿。当年唯一相信你的人，一直暗恋你。现在已是宗门年轻一代的第一人。</p>
            </div>
          </div>
        </div>

        {/* Learning Focus */}
        <div className="bg-white/80 backdrop-blur-md rounded-3xl p-5 shadow-sm border border-white mt-1">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-6 h-6 rounded-full bg-orange-50 flex items-center justify-center">
              <Target size={12} className="text-orange-500" />
            </div>
            <h3 className="font-bold text-[15px] text-[#111827]">学习重点</h3>
          </div>
          <div className="flex flex-wrap gap-2">
            <span className="bg-orange-50/50 text-orange-600 text-[11px] px-3 py-1.5 rounded-full border border-orange-100 font-medium">修仙术语 (Cultivation terms)</span>
            <span className="bg-orange-50/50 text-orange-600 text-[11px] px-3 py-1.5 rounded-full border border-orange-100 font-medium">情感表达 (Emotional expression)</span>
            <span className="bg-orange-50/50 text-orange-600 text-[11px] px-3 py-1.5 rounded-full border border-orange-100 font-medium">冲突对话 (Conflict dialogue)</span>
          </div>
        </div>

      </main>

      {/* Bottom Actions */}
      <motion.div 
        initial={{ y: 100 }}
        animate={{ y: 0 }}
        transition={{ type: "spring", stiffness: 300, damping: 30, delay: 0.2 }}
        className="fixed bottom-[max(env(safe-area-inset-bottom,16px),16px)] left-0 w-full px-4 z-50 pointer-events-none"
      >
        <div className="max-w-md mx-auto flex gap-3 pointer-events-auto">
          <button className="flex-1 bg-white text-gray-600 border border-gray-200 font-bold py-3.5 rounded-2xl shadow-sm flex items-center justify-center gap-2 active:scale-[0.98] transition-all">
            <Edit3 size={16} />
            <span className="text-[14px]">修改设定</span>
          </button>
          <button 
            onClick={() => navigate("/game/roleplay/chat")}
            className="flex-1 bg-gradient-to-r from-pink-500 to-rose-500 text-white font-bold py-3.5 rounded-2xl shadow-xl shadow-pink-500/20 flex items-center justify-center gap-2 active:scale-[0.98] transition-all"
          >
            <span className="text-[14px]">开始故事</span>
            <Play size={14} className="fill-white" />
          </button>
        </div>
      </motion.div>
    </motion.div>
  );
}
