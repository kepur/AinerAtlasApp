import { ArrowLeft, Volume2, Flame, Sparkles } from "lucide-react";
import { useNavigate } from "react-router-dom";

export default function UnifiedHeader({ mode }: { mode?: string }) {
    ...
    <header className="w-full h-14 bg-white/90 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-4 sticky top-0 z-50 pt-[env(safe-area-inset-top,20px)]">
      ...
      <div className="flex flex-col items-center flex-1">
        <div className="flex items-center gap-1.5">
          {isTurtleSoup ? (
            <div className="w-6 h-6 rounded bg-[#f5f3ff] flex items-center justify-center text-[#8b5cf6] font-bold text-[10px]">
              🐢
            </div>
          ) : (
            <span className="text-[#eab308]">✨</span>
          )}
          <h1 className="text-sm font-extrabold text-[#111827]">{isTurtleSoup ? '海龟汤' : 'Story Dialogue'}</h1>
        </div>
        <p className="text-[9px] text-[#6b7280]">
          {isTurtleSoup ? '边玩边学，边推理边练表达' : 'Play the story. Learn the language.'}
        </p>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className={`h-8 rounded-full flex items-center gap-1 px-3 text-white text-[11px] font-bold shadow-sm ${isTurtleSoup ? 'bg-[#10b981]' : 'bg-gradient-to-r from-[#10b981] to-[#059669]'}`}>
          <Flame size={14} className={isTurtleSoup ? 'text-[#fef08a]' : 'text-[#fef08a]'} /> 12 连续学习
        </div>
      </div>
    </header>
  );
